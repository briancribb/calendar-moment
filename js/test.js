...
// https://gist.github.com/adekbadek/d28da825ed964bfd36fb8aa51419b5c8

// API key: AIzaSyCkdl_FrGKOg8wAleNXeHNwBHJ-yoldw84

// https://www.googleapis.com/calendar/v3/calendars/6a8m2n2cif9s72qr0bqqa34vm4@group.calendar.google.com/events?key=AIzaSyCkdl_FrGKOg8wAleNXeHNwBHJ-yoldw84

/*
var CALENDAR_ID = '6a8m2n2cif9s72qr0bqqa34vm4@group.calendar.google.com';
var API_KEY = 'AIzaSyCkdl_FrGKOg8wAleNXeHNwBHJ-yoldw84';
...
var CM = {
	init: function(settings) {
		...
		var targetMoment = moment();//settings.targetMoment;
	...
	build: function(targetMoment) {
		...
		var currentMoment	= targetMoment.clone(),
			headerString	=	'<header class="cm_header">' +
									'<div class="cm_title">' + targetMoment.format("MMMM, YYYY") + '</div>' + 
									'<nav id="cm_nav" class="cm_nav">' +
										'<button class="cm_prev button" disabled><i class="fa fa-chevron-left" aria-hidden="true"></i> Prev</button>' +
										'<button class="cm_now button" disabled>Now</button>' +
										'<button class="cm_next button" disabled>Next <i class="fa fa-chevron-right" aria-hidden="true"></i></button>' +
									'</nav>' +
								'</header>',
		...
		for (var i = 0; i < totalDays; i++) {
			...
			// Add short day of week as a class.
			var dayOfWeek = currentMoment.localeData('en').weekdaysShort(currentMoment);
			if (dayOfWeek) {
				extraClasses += ' ' + dayOfWeek;
			}

			// See if the day is outside of the current month.
			...
			// See when the day happens in relation to today.
			var whenDay = '';

			if ( currentMoment.dayOfYear() < todayMoment.dayOfYear() ) {
				whenDay = ' before';
			} else if ( currentMoment.dayOfYear() > todayMoment.dayOfYear() ) {
				whenDay = ' after';
			} else if ( currentMoment.dayOfYear() === todayMoment.dayOfYear() ) {
				whenDay = ' today';
			}
			extraClasses += whenDay;
			...
			if ( currentMoment.isSame(todayMoment, 'month') && currentMoment.isSame(todayMoment, 'date') ) {
				extraClasses += ' today';
			}
			*/
